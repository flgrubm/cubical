module Cubical.Categories.NewWithFamilies.Structure.Sigma where
...
record Σ-Structure-CwF {ℓ ℓ' ℓTy ℓTm : Level} {C : Category ℓ ℓ'} (cwf : CwF C ℓTy ℓTm) : Type ((ℓ-suc (ℓ-max (ℓ-max (ℓ-max ℓ ℓ') ℓTy) ℓTm))) where
  ...
  Pairs {Δ} A B = Σ[ a ∈ Tm Δ A ] Tm Δ (B ∘Ty ctxExtSubst A IdSubst (idsubst-action A a)) -- (subst⁻ (Tm Δ) (∘ᴾId C tyPresheaf A) a))
